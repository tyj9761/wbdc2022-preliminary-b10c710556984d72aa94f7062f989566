...
from data_helper import create_dataloaders2
...
def train_and_validate(args,fold):
    # 1. load data
    train_dataloader = create_dataloaders2(args)
    ...
    for epoch in range(args.max_epochs):
        ...
        torch.save({'epoch': epoch, 'model_state_dict': model.module.state_dict(), },
                   f'{args.savedmodel_path}/model_epoch_{epoch}_fold{fold}.bin')
        ema.restore()
...
def main():
    ...
    for i in range(9):
        print('第%d个' % i)
        setup_seed(args,fold=i)
        train_and_validate(args,fold=i)
